    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="mb-6 flex items-center gap-3">
        {isCertifiedMode ? (
          <>
            ...
            <h1 className="text-xl font-bold text-gray-800">인증 후기 작성</h1>
            <span className="text-xs px-2 py-0.5 bg-orange-500 text-white font-bold rounded-full">
              실제 방문 인증
            </span>
          </>
        ) : (
          <>
            <PencilLine size={22} className="text-yellow-600" />
            <h1 className="text-xl font-bold text-gray-800">일반 후기 작성</h1>
          </>
        )}
      </div>
      ...
      {isCertifiedMode && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-6 text-sm">
          <p className="text-yellow-700 font-bold mb-1">실제 방문 인증 후기를 작성합니다</p>
      ...
      {certifiedError && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 text-sm text-red-700">
          ...
          <p>{certifiedError}</p>
          <p className="mt-2 text-xs text-red-500">아래에서 일반 후기를 작성할 수 있습니다.</p>
        </div>
      )}
