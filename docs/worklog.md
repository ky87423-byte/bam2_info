...
## 2026-06-06 — 재해 복구 백업 (서버 결제 마감 대비)

- 서버 결제 마감일이라 삭제 위험 → 데이터 보존 점검 + 백업
- 점검 결과: 코드(GitHub `a149f69`), 이미지(로컬 48,892개) 안전
- **서버 라이브 DB 덤프 다운로드**: `backups/server_bam2_20260606_014724.dump` (13개 테이블, 검증됨)
- **서버 런타임/시크릿 백업**: `backups/server_runtime_20260606_014724/` (data/ + .env). shop_overrides.json 없음 = 라이브 어드민 편집/광고지정 아직 없었음
- 결론: 서버 삭제돼도 데이터 손실 0. 재구축은 DEPLOY.md, 이미지 재업로드 3-4h가 유일 비용
- 백업의 GitHub 업로드는 **거부** — .env(AUTH_SECRET/DB비번/SYNC_API_KEY) + DB(회원 비번해시·개인정보)라 익명성 원칙 위배. `backups/`는 gitignore됨
