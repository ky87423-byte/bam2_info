/**
 * 경쟁사 업소 데이터 스크래퍼 (bam2_info/scraper/)
 * - 이미지: ../public/images/imgs/  (Next.js 정적 서빙 — /images/imgs/* URL 즉시 매핑)
 * - 데이터: ./scraped_data/shops.json  (sync action 이 DB 로 upsert)
 * - 환경변수: ../.env  (DATABASE_URL 등 웹사이트와 공유)
 */

const path = require('path');
// 웹사이트 .env 공유 — DATABASE_URL 등 필요 시 process.env 로 사용 가능
...
const CFG = {
    ...
    pauseEvery:    100,
    pauseDuration: 30000,
    imageDir:      path.join(__dirname, '..', 'public', 'images', 'imgs'),  // Next.js public 직행
...
async function scrapeDetail(page, item, idx) {
    ...
    const row = {
        ...
        content,
        area,
        bizType:   item.bizName || '',           // 실제 업종 (건마/오피/술집...)
        category:  item.cat  || '',              // 광역 지역 코드 (legacy 호환)
        category2: item.cat2 || '',              // 세부 지역 코드 (legacy 호환)
        phone:     raw.phone,
        hphone:    raw.hphone,
